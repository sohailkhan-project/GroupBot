...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("tiana", "Aco")
        test = test.replace("Tiana", "Aco")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Aco", "Tiana")
        response = response.replace("aco", "Tiana")
        ...
        response = response.replace("Aco", "Tiana")
        response = response.replace("aco", "Tiana")
        response = response.replace("Luna", "Tiana")
        response = response.replace("luna", "Tiana")
        pro = response
        if not "en" in lan and not lan == "":
            try:
...
__help__ = """
<b> Chatbot </b>
TIANA AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
